      ...
      <table class="table ">
        ...
        <tbody>
          <tr v-for="(element, index) in elements" :key="element.id">
            ...
            <td @dblclick="element.editable = false" @mouseleave="element.editable = true">
              <textarea :disabled="element.editable" v-model="element.name" cols="15" rows="1"></textarea>
            </td>
            <td id="elementSymbol">
              <span> {{ element.symbol }} </span>
            </td>
            <td @dblclick="element.editableWeight = false" @mouseleave="element.editableWeight = true">
            ...
            <td>
              ...
              <b-popover triggers="focus" placement="right" :target="element.name" title="Origin of name">
                {{ element.infoText }}
              </b-popover>
            </td>
          </tr>
